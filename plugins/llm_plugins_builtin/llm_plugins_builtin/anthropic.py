...
class _AnthropicConfig(BaseModel):
    ...
    def _validate(self) -> _AnthropicConfig:
...
class AnthropicLlmPlugin(LlmPlugin):
    ...
    def validate_config(self, config: dict[str, Any]) -> dict[str, Any]:
        cfg = _AnthropicConfig.model_validate(config)
        return cfg.model_dump(mode="json")

    @staticmethod
    def _require_non_empty(settings: dict[str, Any], key: str, label: str) -> Any:
        if key not in settings:
            raise ValueError(f"Anthropic 配置缺失：{label} 为必填项")
        value = settings[key]
