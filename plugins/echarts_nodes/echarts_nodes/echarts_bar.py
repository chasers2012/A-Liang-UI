...
@workflow_node(
    label="ECharts 柱状�?,
    description="�?DataFrame 生成柱状�?ECharts option（echartsy�?,
    ...
    input_sockets=[
        ...
        OptionsNodeParam(
            ...
            default="v",
            label="方向",
            description="v 纵向柱状图，h 条形图（echartsy orient�?,
            options=["v", "h"],
        ),
        BooleanNodeParam(
            "show_legend",
...
class EchartsBarNode:
    def execute(
        ...
        for y in y_list:
            fig.bar(df, x=x_col, y=y, orient=orient)  # type: ignore[arg-type]
