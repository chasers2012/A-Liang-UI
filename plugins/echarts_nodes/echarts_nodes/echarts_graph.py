...
@workflow_node(
    label="ECharts 关系�?,
    description="节点�?+ 边列�?JSON（source/target）生成力导向/环形关系图（echartsy graph�?,
    ...
    input_sockets=[
        ...
        NodeParam(
            ...
            default=None,
            label="边列�?JSON",
            description="数组，每项含 source、target；可�?value �?,
        ),
        StringNodeParam(
            "node_name_column",
            required=False,
        ...
        OptionsNodeParam(
            ...
            default="force",
            label="布局",
            description="force 力导�?/ circular 环形 / none",
...
class EchartsGraphNode:
    def execute(
        ...
        if not isinstance(edges_json, list) or not edges_json:
            raise ValueError("edges_json 必须为非�?JSON 数组")
