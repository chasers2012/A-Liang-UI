...
@workflow_node(
    label="ECharts 旭日�?,
    description="�?DataFrame 层级列生成旭日图（echartsy sunburst�?,
    ...
    input_sockets=[
        ...
        StringNodeParam(
            ...
            default="",
            label="层级�?,
            description="逗号分隔，根→叶，如 Region,Country",
        ),
        StringNodeParam(
            "value_field",
            required=False,
...
class EchartsSunburstNode:
    def execute(
        ...
        if vf:
            fig.sunburst(df, path=path, value=vf)
        else:
            fig.sunburst(df, path=path, value=None)
