...
export const setSchedulerJobLimitAtom = atom(null, (_get, set, value: string) => {
  set(schedulerPageAtom, (s) => ({ ...s, jobLimit: value }));
});

export const refreshSchedulerPageAtom = atom(null, async (get, set) => {
  const { jobLimit } = get(schedulerPageAtom);
  set(schedulerPageAtom, (s) => ({ ...s, loading: true, error: null }));
  try {
    const limit = Number(jobLimit);
    const [tasks, jobs] = await Promise.all([
      listSchedulerTasks(),
      listSchedulerJobs({ limit: Number.isFinite(limit) && limit > 0 ? limit : 50 }),
    ]);
