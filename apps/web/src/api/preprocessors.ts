import type { PreprocessorDetailPublic, PreprocessorSummaryPublic, WorkflowIOSpecPublic } from '@/models';
...
export function getPreprocessor(id: string): Promise<PreprocessorDetailPublic> {
  return apiFetchJson<PreprocessorDetailPublic>(`/preprocessors/${encodeURIComponent(id)}`);
}
...
export function patchPreprocessor(id: string, body: unknown): Promise<PreprocessorDetailPublic> {
  return apiFetchJson<PreprocessorDetailPublic>(`/preprocessors/${encodeURIComponent(id)}`, {
    method: 'PATCH',
    body: JSON.stringify(body),
  });
