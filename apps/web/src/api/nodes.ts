import type { NodeDetailPublic, NodeSummaryPublic, WorkflowDomainNodeVisibilityPublic } from '@/models/nodes/dto';
import { apiFetchJson } from './client';

export function listNodes(domain: string = ''): Promise<NodeSummaryPublic[]> {
  return apiFetchJson<NodeSummaryPublic[]>(`/nodes${domain ? `?domain=${encodeURIComponent(domain)}` : ''}`);
}
...
export function listNodeVisibilityConfigs(): Promise<WorkflowDomainNodeVisibilityPublic[]> {
  return apiFetchJson<WorkflowDomainNodeVisibilityPublic[]>('/nodes/node-visibility');
}
...
export function putNodeVisibilityConfig(
  ...
  return apiFetchJson<WorkflowDomainNodeVisibilityPublic>(`/nodes/node-visibility/${encodeURIComponent(safeDomain)}`, {
    method: 'PUT',
