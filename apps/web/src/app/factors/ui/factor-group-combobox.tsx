...
export function FactorGroupCombobox({ id, value, onValueChange, className }: FactorGroupComboboxProps) {
  ...
  return (
    <Autocomplete.Root items={items} value={value} onValueChange={onValueChange} openOnInputClick>
      <Autocomplete.InputGroup className={cn('relative flex w-full items-stretch', className)}>
        <Autocomplete.Input id={id} placeholder="未分组" autoComplete="off" className={cn(inputClassName, 'flex-1')} />
        ...
        <Autocomplete.Positioner className="z-50 outline-none" sideOffset={4} align="start">
          ...
          >
            ...
            <Autocomplete.List className="outline-none">
              {(group: string) => (
                <Autocomplete.Item key={group} value={group} className={itemClassName}>
                  {group}
                </Autocomplete.Item>
              )}
            </Autocomplete.List>
