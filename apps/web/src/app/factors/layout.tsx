...
/** 供 `/factors` 右侧详情区读取与左侧列表一致的选中项（无 URL id 时默认首项）。 */
export const FactorsLibrarySelectionContext = createContext<string | null>(null);

function parseFactorsRouteId(pathname: string): string | null {
  if (!pathname.startsWith('/factors')) return null;
  const rest = pathname.slice('/factors'.length);
...
function getFactorsEffectiveSelectedId(params: { pathname: string; defaultSelectedId: string | null }): string | null {
  const { pathname, defaultSelectedId } = params;
  if (pathname !== '/factors') return null;
  return defaultSelectedId;
}

export function FactorsLayoutClient({ children }: { children: ReactNode }) {
  ...
  const routeDetailId = parseFactorsRouteId(pathname);
  const effectiveDefaultSelectedId = filteredItems?.[0]?.id ?? null;
  const effectiveSelectedId = getFactorsEffectiveSelectedId({
    pathname,
    defaultSelectedId: effectiveDefaultSelectedId,
  });

  const highlightId = routeDetailId ?? (pathname === '/factors' ? effectiveSelectedId : null);
...
export default function FactorsLayout({ children }: { children: ReactNode }) {
  return <FactorsLayoutClient>{children}</FactorsLayoutClient>;
}
