  ...
  return (
    <span
      className={cn(
        ...
        required ? 'bg-primary/12 text-primary' : 'bg-muted/80 text-muted-foreground',
      )}
    >
      {required ? '必填' : '可选'}
    </span>
...
function PreviewMarkdownBlock({ content }: { content: string }) {
  return (
    <div className="mt-2.5 border-t border-border/40 pt-2.5">
      <MarkdownContent content={content} className="text-[11px] leading-relaxed text-muted-foreground" />
    </div>
  );
}
  ...
  return (
    <Item variant="outline" size="xs" className="bg-card/40 shadow-none">
      <ItemContent className="gap-1.5">
        ...
        {socket.description?.trim() ? <PreviewMarkdownBlock content={socket.description.trim()} /> : null}
      </ItemContent>
    </Item>
  );
}
  ...
  return (
    <Item variant="outline" size="xs" className="bg-card/40 shadow-none">
      <ItemContent className="gap-1.5">
        ...
        {item.description?.trim() ? <PreviewMarkdownBlock content={item.description.trim()} /> : null}
      </ItemContent>
    </Item>
  );
}
