...
export function EvaluationProfileFormPage(props: Props) {
  ...
  if (templateLoading) {
    return (
      <Page title="新增评价方案">
        <p className="text-sm text-muted-foreground">{EVALUATION_WORKFLOW_TEMPLATE_LOADING_TEXT}</p>
      </Page>
    );
  }
  ...
  return (
    ...
    >
      <form id={formId} className="flex min-h-0 flex-1 flex-col gap-6" onSubmit={(e) => void onSubmit(e)}>
        ...
        <ProfileWorkflowEditorBlock workflow={workflow} canvasKey={canvasKey} canvasRef={canvasRef} />
      </form>
    </Page>
  );
}
