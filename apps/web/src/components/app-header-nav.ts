...
export function useAppHeaderBreadcrumbs(): PageBreadcrumbItem[] {
  ...
  if (segments.length === 0) return [{ label: SIDEBAR_NAV[0]?.title ?? '首页' }];

  return segments.map((segment, index) => {
    const href = `/${segments.slice(0, index + 1).join('/')}`;
    const isLast = index === segments.length - 1;
    const matchedMain = SIDEBAR_NAV.find((item) => item.url === href);
    const matchedSub = SIDEBAR_NAV.flatMap((item) => item.items ?? []).find((item) => item.url === href);
    return {
