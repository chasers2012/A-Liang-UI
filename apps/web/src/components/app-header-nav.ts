...
function withMenuSection(pathname: string, items: PageBreadcrumbItem[]): PageBreadcrumbItem[] {
  if (pathname === FACTOR_ROOT_HREF || pathname === DATA_ROOT_HREF) {
    return items;
  }

  if (items[0]?.href === FACTOR_ROOT_HREF || items[0]?.href === DATA_ROOT_HREF) {
    return items;
  }
...
function factorsLibraryHeaderBreadcrumbs(pathname: string): PageBreadcrumbItem[] | null {
  ...
  if (edit) {
    ...
    return [{ href: FACTOR_LIBRARY_LIST, label: '因子库' }, { href: base, label: '因子详情' }, { label: '编辑' }];
  }

  const detail = /^\/factors\/library\/([^/]+)$/.exec(pathname);
  if (detail) {
    return [{ href: FACTOR_LIBRARY_LIST, label: '因子库' }, { label: '因子详情' }];
  }
...
function prefixSectionBreadcrumbs(pathname: string, basePath: string, listLabel: string): PageBreadcrumbItem[] | null {
...
/** 主内容顶栏面包屑 */
export function buildAppHeaderBreadcrumbs(pathname: string): PageBreadcrumbItem[] {
  ...
  const metrics = standardResourceBreadcrumbs(pathname, FACTOR_METRICS_LIST, '节点', '详情');
  ...
  const profiles = standardResourceBreadcrumbs(pathname, FACTOR_PROFILES_LIST, '评价方案', '详情');
  if (profiles) return withMenuSection(pathname, profiles);

  const datasources = standardResourceBreadcrumbs(pathname, DATA_DATASOURCES_LIST, '数据源', '数据源详情');
  if (datasources) return withMenuSection(pathname, datasources);

  const dataSetsCrumbs = standardResourceBreadcrumbs(pathname, DATA_DATA_SETS_LIST, '数据集', '数据集详情');
  ...
  if (pathname === '/chat/archived') {
    return withMenuSection(pathname, [{ href: '/', label: '对话' }, { label: '已归档会话' }]);
  }
