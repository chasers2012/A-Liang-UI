...
import { archiveChatAtom } from '@/models/chat';

import { ActiveSessionSnapshotTrigger } from './active-session-snapshot-trigger';

export const ArchiveButton = memo(function ArchiveButton({ disabled }: { disabled: boolean }) {
  ...
  return (
    <>
      <ActiveSessionSnapshotTrigger disabled={disabled} onRequestOpen={onRequestOpen} ariaLabel="归档当前会话">
      ...
      <ConfirmDialog
        ...
        description={
          <div className="space-y-2">
            <p className="text-sm text-foreground">确认归档会话「{payload?.title ?? ''}」？</p>
