...
export const ChatMessageCollapsible = memo(function ChatMessageCollapsible({ mid }: { mid: string }) {
  useAtomValue(segementOpenEffect);
  ...
  return (
    <div className="w-full min-w-0 contain-[layout]" id={mid}>
      <div className="group/trigger mb-3 z-10 sticky top-3 flex w-full min-w-0 items-start gap-2 bg-background text-left outline-none rounded-lg">
        <button
          type="button"
          className="inline-flex shrink-0 pt-3"
          onClick={toggleOpen}
        ...
        <ChatMessageUserContent mid={mid} text={messageText} />
      </div>
