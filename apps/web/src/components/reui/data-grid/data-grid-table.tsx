'use client';
/* eslint-disable complexity, react-hooks/set-state-in-effect */
...
function getDataGridResizeEventClientX(event: DataGridResizeStartEvent | DataGridResizeDocumentEvent) {
...
function DataGridTableViewport({
  ...
  const fillWidth =
    props.tableLayout?.columnsResizable && containerWidth > 0 ? Math.max(0, containerWidth - table.getTotalSize()) : 0;
...
function DataGridTableResizeIndicator({ viewportElement }: { viewportElement: HTMLDivElement | null }) {
  const { props, table } = useDataGrid();
...
function DataGridTableFootRow({ children }: { children: ReactNode }) {
  const { props } = useDataGrid();
  return (
    <tr className={cn('bg-muted/40 dark:bg-background', props.tableLayout?.cellBorder && '*:last:border-e-0')}>
      {children}
...
function DataGridTableEmpty() {
  ...
  return (
    <tr>
      <td colSpan={Math.max(visibleColumnCount, 1)} className="text-muted-foreground text-sm py-6 text-center">
