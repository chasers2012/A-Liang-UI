...
function DataGridTableVirtualSpacer({ columnCount, height }: { columnCount: number; height: number }) {
...
function DataGridTableVirtualStatusRow({
  ...
  return (
    <tr>
      <td colSpan={columnCount} className={cn('text-muted-foreground py-4 text-center text-sm', className)}>
        {children}
      </td>
    </tr>
  );
...
function DataGridTableVirtualBody<TData>({
  ...
  if (isVirtualizationEnabled) {
    ...
    if (trailingSpacerHeight > 0) {
      renderedRows.push(
        <DataGridTableVirtualSpacer key="virtual-spacer-end" columnCount={columnCount} height={trailingSpacerHeight} />,
      );
    }
  } else {
  ...
  if (showCompleteRow) {
    renderedRows.push(
      <DataGridTableVirtualStatusRow key="virtual-status-complete" columnCount={columnCount} className="py-3 text-xs">
        {allRowsLoadedMessage}
      </DataGridTableVirtualStatusRow>,
    );
  }
...
function DataGridTableVirtual<TData>({
  ...
  const handleViewportRef = useCallback((node: HTMLDivElement | null) => {
    setViewportElements({
      ...
      scrollElement: (node?.closest('[data-slot="scroll-area-viewport"]') as HTMLElement | null) ?? node,
    });
  }, []);
  ...
  return (
    <DataGridTableViewport
      ...
      style={usesExternalScrollArea ? undefined : { height, overflow: 'auto', position: 'relative' }}
    >
      <DataGridTableBase>
        {renderHeader && (
          <DataGridTableHead>
