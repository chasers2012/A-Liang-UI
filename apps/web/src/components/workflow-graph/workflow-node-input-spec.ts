...
export function isWireInputSpec(s: WorkflowNodeInputSpec): boolean {
  ...
  return !s.render_type || s.render_type === 'socket' || s.render_type === 'appendable';
}

export function inputSpecToNodeParamModel(s: WorkflowNodeInputSpec): NodeParamModel {
  return {
    key: s.name,
    ...
    json_schema: s.json_schema ?? undefined,
    ui_schema: s.ui_schema ?? undefined,
  };
}
