...
from app.visibility.controller import (
    ...
    upsert_domain_node_visibility_config,
)
...
def list_domain_nodes() -> list[WorkflowDomainNodeVisibilityPublic]:
    data = list_domain_node_visibility_configs()
    return [
...
def get_domain_nodes(domain: str) -> WorkflowDomainNodeVisibilityPublic:
    hidden_node_ids = get_domain_node_visibility_config(domain)
...
def is_domain_node_allowed(domain: str, node_id: str) -> WorkflowDomainNodeValidationPublic:
    return WorkflowDomainNodeValidationPublic(
        domain=domain.strip(),
        node_id=node_id,
        allowed=is_node_visible_in_domain(domain, node_id),
    )
