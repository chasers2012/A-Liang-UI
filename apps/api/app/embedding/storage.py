...
from app.embedding.plugins import get_embedding_plugin, list_embedding_plugins

ROOT_KEYS: tuple[str, ...] = ("embedding_provider",)
_RESERVED_KEYS = frozenset((*ROOT_KEYS, "embedding_providers"))


def inflate_embedding_config_for_ui(stored: dict[str, Any]) -> dict[str, Any]:
    """
    Flatten selected provider settings onto the root for RJSF editing.

    Persisted shape:
    - ``embedding_provider`` at root
    - provider fields under ``embedding_providers.<provider>``
    """
    data = dict(stored or {})
    provider = str(data.get("embedding_provider") or "").strip()
    providers = data.get("embedding_providers")
...
def _get_provider(stored: dict[str, Any]) -> str:
    provider_value = stored.get("embedding_provider")
...
def _load_existing_providers(existing: dict[str, Any] | None) -> dict[str, Any]:
    if isinstance(existing, dict) and isinstance(existing.get("embedding_providers"), dict):
        return dict(existing["embedding_providers"])
    return {}


def _merge_incoming_providers_map(providers: dict[str, Any], incoming: dict[str, Any]) -> None:
    incoming_providers = incoming.get("embedding_providers")
...
def _list_all_provider_field_keys() -> set[str]:
    keys: set[str] = set()
    for plugin in list_embedding_plugins():
...
def split_embedding_config_for_storage(
    incoming: dict[str, Any],
    *,
    existing: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """
    Split incoming embedding config into per-provider buckets for persistence.

    - Keeps only ``embedding_provider`` at root.
    - Stores provider fields under ``embedding_providers.<provider>``.
    ...
    if providers:
        out["embedding_providers"] = providers
    return out
