...
from app.packages.data_sync.schemas import (
    DataSyncRunResult,
...
def _frames_by_target_from_workflow(
    ...
    if isinstance(frames_out, pd.DataFrame):
        for target_id, _inst in targets:
            frames_by_target[target_id] = frames_out
        return frames_by_target
    for target_id, _inst in targets:
        val = workflow_out.get(target_id)
        if not isinstance(val, pd.DataFrame):
            raise ValueError(f"工作流缺少目标输出: {target_id}")
        frames_by_target[target_id] = val
    return frames_by_target
...
def _write_sync_to_targets(
    ...
    for target_id, inst in targets:
        df_out = frames_by_target[target_id]
        date_col = inst.date_column
