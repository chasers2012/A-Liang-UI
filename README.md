...
| 数据存储 | 配置、数据库、因子源码、知识库等默认保存在 `~/.a-liang-ui`，可用 `WORKSPACE_ROOT` 指定其他路径                 |
...
浏览器访问 [http://127.0.0.1:8000](http://127.0.0.1:8000)（页面在 `/`，接口在 `/api`）。`NEXT_PUBLIC_BACKEND_API` 在 `docker-compose.yml` 中配置，默认同源 `/api`。数据持久化在 Docker volume `a-liang-ui-data`（容器内 `/data`）。
...
| 变量                      | 作用域 | 说明                                                                                               |
| ------------------------- | ------ | -------------------------------------------------------------------------------------------------- |
| `NEXT_PUBLIC_BACKEND_API` | 前端   | API 基址，本地开发默认 `http://127.0.0.1:8000/api`；Docker 默认同源 `/api`，勿以 `/` 结尾          |
| `WORKSPACE_ROOT`          | 后端   | 数据根目录，默认 `~/.a-liang-ui`                                                                   |
